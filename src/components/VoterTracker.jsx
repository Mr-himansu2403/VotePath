...
const VoterTracker = ({ tracker, setTracker, title, lang }) => {
  ...
  const handleListen = () => {
    ...
    const pending = tracker.filter(t => !t.completed).map(t => t.task).join('. ');
    const text = `You have completed ${completedCount} out of ${tracker.length} steps. Your remaining tasks are: ${pending}`;
    speak(text, lang);
  };

  const completedPct = Math.round((tracker.filter(t => t.completed).length / tracker.length) * 100);
    ...
    <div className="view-container">
      ...
      <div className="tracker-progress-card">
        ...
        <div className="progress-track" style={{ height: '8px' }}>
          <div className="progress-fill" style={{ width: `${completedPct}%`, background: 'var(--gold)' }}></div>
        </div>
      </div>
        ...
        {tracker.map((item) => (
          ...
          >
            <div className="tracker-check">
              {item.completed ? <CheckCircle2 className="icon-check" /> : <Circle className="icon-circle" />}
            </div>
